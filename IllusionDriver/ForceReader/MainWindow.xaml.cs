...
namespace ForceReader
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public static List<ExpData> valuesToWrite = new List<ExpData>();
        ...
        public static void setRecordValuesMode(bool recording, string fileName) {
            if (recording) {
               ...
               // System.IO.File.WriteAllText(currentFilePath, valuesToWrite.ToString());
            }
        }
        ...
        public static void saveDataToFileAndClearList() {
            using (TextWriter tw = new StreamWriter(currentFilePath))
            {
                ...
                {
                    tw.WriteLine(string.Format("{0}, {1}, {2}, {3}", (data.fx).ToString(), data.fy.ToString(), data.fz.ToString(), data.magnitude.ToString()));
                }
            }
            valuesToWrite = new List<ExpData>();
        }
        ...
        {
            double[] higherCountMarks = { 0.4, 0.8, 1.2, 1.6, 2 };
        ...
        private void ProcessForce()
        {
            ...
            if (actuator.IsOpen)
            {
                ...
                if (bendMag < bendBottomOutThresh) //&& !CheckIfWallsHit())
                {
                    bendBottomOut = false;
                    if (dist > bendThresh)
                    {
                        ...
                        isOutofBound = forceIsOutOfBounds();
                        if (!isOutofBound)
                        {
                           PlayGrain();
                       }
        ...
        private void EncodeTactilePatterns()
        {
            ...
            {               
                ...
                {
                    if (leftMarks[i] < prevHapticForce.fx && leftMarks[i] > hapticForce.fx)
                    {
        ...
        {
            return force.fx <= -10 || force.fx >= 10 || force.fy <= -10 || force.fy >= 10;
        }
        ...
        List<double> leftMarks, rightMarks, upMarks, downMarks;
        ...
        //private void debugBox_TextChanged(object sender, System.Windows.Controls.TextChangedEventArgs e)
        //{
        ...
        //{
        //    leftMarks = new List<double>();
        //    rightMarks= new List<double>();
        //    upMarks= new List<double>();
        //    downMarks= new List<double>();

        //    string[] marks = settingBox.Text.Split('\n');

        //    string[] marks1 = (marks[0].Split(':')[1]).Split(',');
        //    string[] marks2 = (marks[1].Split(':')[1]).Split(',');
        //    string[] marks3 = (marks[2].Split(':')[1]).Split(',');
        //    string[] marks4 = (marks[3].Split(':')[1]).Split(',');

        //    foreach (string s in marks1)            
        //        leftMarks.Add(double.Parse(s));
        //    foreach (string s in marks2)
        //        upMarks.Add(double.Parse(s));
        //    foreach (string s in marks3)
        //        downMarks.Add(double.Parse(s));
        //    foreach (string s in marks4)
        //        rightMarks.Add(double.Parse(s));
        //}

        private void GenerateHapticMarks(int direction, int count)
        {
  ...
  public class Force
    {
        ...
        public void SetForce(Response resp)
        {
            ...
            if (Math.Sqrt(fx*fx+fy*fy) > dirThresh)
            {
                isRoaming = false;
                if(!dirSet)
                {
            ...
            if (!MainWindow.recordValues) return;
