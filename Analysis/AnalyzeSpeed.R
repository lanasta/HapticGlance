...
analyzeThresholdPoints <- function(mydata, direction, speed) {
...
plotMagnitude <- function(direction) {
  ...
  for (filename in myFiles) {
    ds <- read.csv(filename)
    mydata <- ds[ds$Magnitude > 0.10,]
    speedData <- ds[1:min(which(ds$Magnitude >=1)),]
    timestamp <- speedData$Timestamp
    
    speed <- as.numeric(1/((tail(timestamp, n=1))/1000))
    cat("\n Speed: ",speed)
    averageSpeed <- c(averageSpeed,as.numeric(1/((tail(timestamp, n=1))/1000)))
    ...
    errorAnglesList <- analyzeThresholdPoints(mydata, direction, speed)
